...
Install [docker](https://docs.docker.com/get-docker/) and [docker-compose](https://docs.docker.com/compose/install/) yourself, then run the following to set up first Odoo instance @ `localhost:10017` (default master password: `Hilal@123`):

``` bash
curl -s https://raw.githubusercontent.com/HilalRashad/odoo-17-docker-compose/master/run.sh | sudo bash -s odoo-one 10017 20017
```
and/or run the following to set up another Odoo instance @ `localhost:11017` (default master password: `Hilal@123`):

``` bash
curl -s https://raw.githubusercontent.com/HilalRashad/odoo-17-docker-compose/master/run.sh | sudo bash -s odoo-two 11017 21017
```
...
* Default database password (**admin_passwd**) is `Hilal@123`, please change it @ [etc/odoo.conf#L60](/etc/odoo.conf#L60)
